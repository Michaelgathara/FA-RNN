...
class NFA:
    ...
    def move(self, qs, x):
        ...
        reachable = set()
        for l in self.delta:
            if l[0] in qs and l[1] == x:
                reachable = reachable | {l[2]}
        return reachable
    ...
    def NFA_to_DFA(self):
        ...
        """
        init a list
        for every state in self.F 
            check if that state is in startset:
                if so add to the list you init up above

        if the length of above list is greater than one:
            dfa.F = {start}
        else:
            dfa.F = set()

        marked = {start}
        while marked is not equal to set()
            pop the marked set and set it to a variable (for example, "nextUp")
            for characters in dfa.Sigma
                set a variable (say "afterMove") equal to the epsilonClosure of self.move(nextUp.name, character)
                if the afterMove part is not a set()
                    set a new var (say toAdd) equal to State(frozenset(afterMove))
                    if toAdd is not in dfa.Q:
                        add it to dfa.Q
                        add it to marked

                        init a new list 
                        for everything in afterMove:
                            if that thing is in self.F:
                                add the toAdd state you created above to dfa.F
                    dfa.delta.add((nextUp, character, toAdd))
        return dfa
        """
        return dfa 
