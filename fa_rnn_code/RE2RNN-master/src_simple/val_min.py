...
def val(model, intent_dataloader, epoch=0, mode='TRAIN', config=None, i2in=None, logger=None,  criterion=torch.nn.CrossEntropyLoss()):
    if intent_dataloader is None:
    ...
    with torch.no_grad():
        for batch in intent_dataloader:
            ...
            pad_token_index = 0  # Replace 0 with your actual pad token index
            src_mask = (x_forward != pad_token_index).transpose(0, 1)
