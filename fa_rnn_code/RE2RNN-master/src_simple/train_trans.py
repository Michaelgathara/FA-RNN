...
def get_init_params(config, in2i, i2in, t2i, automata_path):
    ...
    # for padding
    pretrain_embed_extend = np.append(pretrained_embed, np.zeros((1, config.embed_dim), dtype=np.float64), axis=0)
    V_embed_extend = np.append(V_embed, np.zeros((1, rank), dtype=np.float64), axis=0)
...
def train_fsa_rnn(args, paths):
    ...
    # if args.optimizer == 'SGD':
    #     optimizer = torch.optim.SGD(model.parameters(), lr=args.lr, weight_decay=0)
    if args.optimizer == 'ADAM' or args.optimizer == 'SGD':
        optimizer = torch.optim.Adam(model.parameters(), lr=args.lr, weight_decay=0)
    ...
    for epoch in range(1, args.epoch + 1):
        ...
        for batch in pbar_train:
            ...
            x_forward = batch['x_forward']  # Input sequences
            label = batch['i'].view(-1)     # Labels for each sequence

            # Create a mask for the Transformer
            # The mask should indicate where the sequence is padding
            src_mask = (x_forward != t2i['<pad>']).transpose(0, 1)  # Assuming t2i['<pad>'] is your padding token index
            ...
            scores = model(x_forward, src_mask)
